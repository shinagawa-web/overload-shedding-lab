...
const header = 'stage,condition,concurrency,knob,rps,p50,p99,errors,timeouts,non2xx,cpu_pct,lag_p99'
const rows = [header]

function rowFrom(s) {
  return [s.label, s.condition, s.concurrency, s.knob, s.rps, s.p50, s.p99, s.errors, s.timeouts, s.non2xx ?? 0, s.cpu_pct ?? 'n/a', s.lag_p99 ?? 'n/a'].join(',')
}
