...
条件 B: /sync-cpu p99 が 205ms → 1801ms に跳ね上がった。処理数上限（semaphore=10）をつけることで 50 接続のうち 40 本が goroutine ごとブロック待ちになり、p99 が悪化した。「処理数上限を付けると腕 A より p99 が悪化する」は確認された。/light は 5ms で安定。goroutine はブロック中でも他の goroutine に干渉しない。
